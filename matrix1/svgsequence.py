...
def ConvertToRawFiles():
  for pngfilename in pngfiles:
    ...
    subprocess.run(['convert', pngfilename, rgbafilename])
...
def MergeRawFiles():
  ...
  for rgbafilename in rgbafiles:
    ...
    for i in range(64*32):
      ...
      a = pixel[3]
      for color in range(3):
        blended_image[i*4 + color] = min(255, int(pixel[color] * (a / 255) + (blended_image[i*4 + color] * 0.66) * ((255 - a) / 255)))
    inputfile.close()
    outputfile = open(rgbafilename, 'wb')
    outputfile.write(bytes(blended_image))
    outputfile.close()
    subprocess.run(['convert', '-size', '64x32', '-depth', '8', rgbafilename, f"{filename}-merged.png"])
